...
- 리스트 행 ≥52px, 터치 타깃 ≥44px. 상단 네비바 + **하단 탭 바**(메인/레시피 북, 헤더 링크와 병행) + 카드 탭으로 여는 제안 상세 1개(축복받은 단일 push). 화면은 메인·레시피 북 + 제안 상세이며, 코어 루프는 탭 셸 안에서 흐른다(ADR-0007이 ADR-0001을 역전).
...
- **헤더**: 상단 네비바 + 우측 레시피 북 링크 1개(텍스트/아이콘). **하단 탭 바**(메인/레시피 북, ADR-0007).
...
- **확인 다이얼로그**: surface 배경, radius 16(카드와 같은 단), flat. 되돌릴 수 없는 파괴적 동작에만 쓴다 — 되돌릴 수 있으면 실행취소 토스트가 맞다. 액션은 좌 취소·우 실행 2개까지. Material 기본값(elevation 6·radius 28·`surfaceContainerHigh`)은 이 문서의 flat·라운드 스케일·쿨 뉴트럴과 어긋나므로 **테마에서 덮는다**.
