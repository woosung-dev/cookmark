...
> wayfinder 티켓 [#3](https://github.com/woosung-dev/cookmark/issues/3) · deep-research · 2026-07 (검증 완료)
> 검증 이력(라운드별 경위)은 이슈 [#3](https://github.com/woosung-dev/cookmark/issues/3) 코멘트 참조.
> 표기 — ✅ 검증됨 / ⚠️ 미검증 / ❌ 기각

## 핵심 결론

1. **음식·재료 인식에서 Gemini 계열이 벤치마크 최상위다**(✅). 모델 1순위 후보는 Gemini Flash 계열 — 성능·원가·지연 모두에서 유리.
2. **다중 품목 이미지에서 정확도가 크게 떨어진다**(✅) — 냉장고 사진은 정확히 이 최악 조건이다. P2 킬 기준(사진당 수동 수정 5개)의 현실성은 문헌으로 결론 낼 수 없고, **P1 스파이크(실물 사진)가 반드시 필요**하다.
3. **사진 1장당 원가는 병목이 아니다**(✅) — Gemini 2.5 Flash 기준 이미지 1장당 약 $0.006('Best' 구성 4회 호출 합산), 단순 1회 호출이면 1센트 미만.
4. **유튜브 재료 추출은 설명란 파싱 → 자막(yt-dlp) → 영상 이해 순의 폴백 체인이 합리적이다.** 영상 입력은 기본 해상도 **초당 약 300토큰**(프레임 258 + 오디오 32), 저해상도 초당 약 100토큰 — 10분 영상 ≈ 18만(기본)/6만(저해상도) 토큰으로 영상 직접 처리가 가장 비싸다(✅).
5. **벤더 광고와 실사용의 격차가 실재한다**(✅) — "95% 정확도" 광고 대비, 전용 하드웨어(삼성 Bespoke)조차 수동 수정이 일상. 체크박스 수정을 실패가 아니라 **1급 UX로 설계**해야 한다.
...
- **DiningBench 세밀 분류** — Gemini 3 Flash Preview 81.83%, Gemini 3 Pro 81.55% vs GPT-4o 65.26%, Claude Sonnet 4.5 54.40%. ([arXiv 2604.10425](https://arxiv.org/html/2604.10425))
...
### 원가·토큰
- **JFB 실측(2025-08, Table 2)** — 호출당 Gemini 2.5 Flash $0.0014/13.5초, GPT-4o $0.0065/10.3초, Gemini 2.5 Pro $0.0225/28.1초. 'Best' 구성은 이미지당 4회 호출 → **이미지 1장당 Flash ≈ $0.006**. ([arXiv 2508.09966](https://arxiv.org/html/2508.09966v1))
- **Gemini 공식 가격** — 2.5 Flash 입력 $0.30/M(텍스트·이미지·비디오)·출력 $2.50/M, 2.5 Flash-Lite $0.10/$0.40. ([가격](https://ai.google.dev/gemini-api/docs/pricing))
...
- **영상 토큰 산정** — 기본 해상도 초당 약 300토큰(프레임 258+오디오 32), 저해상도 초당 약 100토큰. ([문서](https://ai.google.dev/gemini-api/docs/tokens))

### 실패 모드·실사용 현실
- **냉장고 인식 실패 모드** — "occlusions, variable distortions, and complex backgrounds … require frontal and close-up images". ([PMC](https://pmc.ncbi.nlm.nih.gov/articles/PMC11652515/))
...
- **오인식의 실제 사례** — 삼성 Bespoke가 손가락의 반창고를 채소로 라벨링, 크림치즈를 "Philadelphia Plant-based"로 오인, "Other times it seemingly just guesses". 전용 하드웨어조차 이 수준 — 인식 대상도 신선 37종·포장 50종 온디바이스 제한. CES 2026 Gemini 업그레이드(2,000종)로 완화 예정이라 시효성 주의. ([techbriefly](https://techbriefly.com/2026/05/11/samsung-expands-smart-fridge-food-recognition-to-2000-items/) · [삼성 뉴스룸](https://news.samsung.com/global/samsung-to-unveil-ai-vision-built-with-google-gemini-at-ces-2026))
...
- 냉장고 내부 LED 색온도·밝기 편차, 소형 품목의 저픽셀 문제(1080p+ 필요), 시간 경과에 따른 식품 외관 변화(변색·시듦)가 정적 이미지 모델을 흔든다. ([basic.ai](https://www.basic.ai/blog-post/computer-vision-for-smart-fridges-how-it-works-models-data-and-annotations))
...
- 전용 CNN 냉장고 시스템은 "내부 1장 사진"을 회피하고 출입 순간 개별 촬영 방식을 씀 — 1장 시나리오의 난이도 방증. 참고치 — BroadFPN-YOLACT는 60~100cm 소형 물체 95.0% mAP(표준 YOLACT 72.3%). ([Frontiers](https://www.frontiersin.org/journals/artificial-intelligence/articles/10.3389/frai.2024.1442948/full))
- 단일 2D 사진에선 양 추정 불가, 숨은 재료(기름·설탕) 인식 불가. ([arXiv 2511.08215](https://arxiv.org/html/2511.08215))

## 기각된 주장 ❌ (근거로 쓰지 말 것)

- "조리 스타일 식별은 최고 모델도 ~50% EWR에 그쳐 조리된 반찬 인식이 훨씬 어렵다" — 기각(1-2, 수치 해석 이의). 반찬 인식 난이도 자체는 P1 스파이크에서 실측할 것.
- "영상 입력은 263토큰/초" — 원문에 없는 수치(정확한 값은 위 ✅ 참조).
- "유사 포장 오인(플레인 요거트 vs 사워크림) 사례" — 기각(0-3).

## 냉파에의 시사점

- **모델 선택** — 1순위 Gemini Flash 계열(2.5 Flash 또는 3 Flash Preview). 디자인 문서의 "Gemini Vision 1개" 결정과 정합. 원가는 병목 아님.
- **P1 스파이크 설계 입력** — 다중 품목 급락(✅)과 가림·용기 실패 모드 때문에, 스파이크는 "정돈된 냉장고"가 아니라 **실제 상태(겹침·반찬통 포함)** 사진으로 해야 의미가 있다. 수동 수정 개수를 세는 것이 곧 P2 킬 기준 실측이며, 잴 것은 "수정 0회 가능한가"가 아니라 "수정이 5개/장 이내로 수렴하는가"다.
- **체크박스 수정 = 1급 UX** — 전용 하드웨어도 수동 수정이 일상이므로, 수정을 실패 상태가 아니라 플로우의 일부로 설계한다.
- **유튜브 재료 추출 전략** — ①설명란 파싱(무료) → ②자막 yt-dlp(무료) → ③영상 이해(고비용)의 폴백 체인. MVP의 "추출 실패 시 수동 입력 폴백"과 결합하면 충분.

## 출처

[arXiv 2504.06925(FoodNExTDB)](https://arxiv.org/html/2504.06925v1) · [arXiv 2604.10425(DiningBench)](https://arxiv.org/html/2604.10425) · [arXiv 2508.09966(JFB)](https://arxiv.org/html/2508.09966v1) · [arXiv 2312.14991(FoodLMM)](https://arxiv.org/html/2312.14991v1) · [arXiv 2511.08215](https://arxiv.org/html/2511.08215) · [arXiv 2406.16469](https://arxiv.org/pdf/2406.16469) · [arXiv 2509.07400](https://arxiv.org/abs/2509.07400) · [PMC11652515](https://pmc.ncbi.nlm.nih.gov/articles/PMC11652515/) · [Frontiers AI](https://www.frontiersin.org/journals/artificial-intelligence/articles/10.3389/frai.2024.1442948/full) · [Gemini 가격](https://ai.google.dev/gemini-api/docs/pricing) · [이미지 이해](https://ai.google.dev/gemini-api/docs/image-understanding) · [토큰](https://ai.google.dev/gemini-api/docs/tokens) · [영상 이해](https://ai.google.dev/gemini-api/docs/video-understanding) · [Claude 가격](https://platform.claude.com/docs/en/about-claude/pricing) · [OpenAI 가격](https://developers.openai.com/api/docs/pricing) · [Artificial Analysis](https://artificialanalysis.ai/models) · [AI Hub 한국 음식 데이터셋](https://aihub.or.kr/aihubdata/data/view.do?dataSetSn=79) · [yt-dlp 자막 블로그](https://skipthewatch.com/blog/yt-dlp-youtube-subtitles) · [YouTube→레시피 블로그](https://angel-baez.com/blog/youtube-to-recipe-with-ai/) · [fitham.ai](https://fitham.ai/en/blog/ai-fridge-scanner/) · [basic.ai](https://www.basic.ai/blog-post/computer-vision-for-smart-fridges-how-it-works-models-data-and-annotations) · [Engadget(Bespoke)](https://www.engadget.com/home/kitchen-tech/samsung-bespoke-fridge-with-ai-review-all-the-bells-and-whistles-140000099.html) · [techbriefly](https://techbriefly.com/2026/05/11/samsung-expands-smart-fridge-food-recognition-to-2000-items/) · [삼성 뉴스룸](https://news.samsung.com/global/samsung-to-unveil-ai-vision-built-with-google-gemini-at-ces-2026)
