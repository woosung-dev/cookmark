...
- [x] LLM seam — `src/services/ai_processing.py`(BaseLLMService·extract만·ExtractionUnavailable·GeminiLLMService·get_llm_service) + `src/common/prompts.py` + Settings `GEMINI_API_KEY`/`GEMINI_MODEL` + `tests/llm.py` 페이크 + conftest 배선
