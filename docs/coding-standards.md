...
**알면서 남긴 부채다.** 파일럿 D0(2026-07-20)가 코앞이고 코드는 이미 배포·검증돼 돌아간다.
파일럿은 제품 질문 2개를 재는 장치이지 아키텍처를 재지 않는다. 정합 리팩터는 **파일럿 후**
별도 트랙이다([#38](https://github.com/woosung-dev/cookmark/issues/38)). E2E 144건이 행동을 고정하고 있어 안전망은 있다.
