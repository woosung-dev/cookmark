...
- Flutter(Dart) 단일 코드베이스. **파일럿 배포 타깃 = 네이티브 Android APK**(ADR-0011이 ADR-0005의 "Web 빌드로 배포"를 역전 — 스택은 무변경, 역전 대상은 배포 타깃뿐). 웹 빌드는 개발·E2E 편의로 **유지**된다(네이티브 관통 증명 전 삭제 금지 — 남은 웹 코드는 결함이 아니라 의도된 잔존, 정본은 ADR-0011 웹 폐기 시퀀싱).
- 로그인·서버 DB 없음 — 클라이언트 로컬 영속이 유일한 영속층. **네이티브에서는 `shared_preferences`(SharedPreferences), 웹 빌드에서는 브라우저 스토리지가 백엔드다.** 네이티브에서는 로컬 데이터 연속성이 **릴리스 서명에 묶인다** — 같은 키스토어로 서명해야 재설치가 업데이트로 처리되어 데이터가 보존된다(다른 키 = 전멸, ADR-0011·#132). (참고 — 서버 레시피 북은 별개 트랙 ADR-0009이고 파일럿 앱은 무의존이다.)
- LLM 프록시(인식·추출·매칭)는 앱과 분리된 서버리스 함수 — API 키는 클라이언트에 두지 않는다. 네이티브엔 same-origin이 없어 빌드 시 `--dart-define=COOKMARK_API_BASE=…`로 프록시 절대 URL을 주입한다(ADR-0011).
...
- **integration_test** — E2E. 파일럿은 네이티브 APK로 배포하지만 **E2E는 아직 Web 타깃으로 돈다**(네이티브 타깃 전환은 미결 — 파일럿 후 정리 트랙, 지도 #129 잔여 fog). 웹 빌드가 살아 있어야 이 게이트가 돌고, 이는 ADR-0011의 웹 폐기 시퀀싱과 정합한다. 실기기 전용 항목(카메라·실사진·카톡 설치)은 #65 D0 스모크 몫이다. 검증의 정본은 E2E이고, 유닛은 보완이다.
