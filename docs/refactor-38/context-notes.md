...
### Riverpod ↔ freezed 순서 → **freezed 먼저 확정 (2026-07-16 파운더 결정)**
MainController 532줄 복합 상태기계(phase enum + 10+ 필드 + 세대 가드·stale)를 Notifier로 옮기려면 상태를 하나의 클래스로 묶어야 하고, freezed 없이 수동 copyWith는 필드가 많아 지옥이라 순서를 뒤집었다. 새 순서 — **freezed(domain 모델 + 상태 클래스) → Riverpod Notifier + Failure sealed → 위젯 Consumer → 유닛 154 재작성.** build_runner는 둘 다 선행(이미 배선). riverpod deps도 이미 추가됨(pubspec).
